...
/* الترويسة — شعار فحمي منسجم (لا تدرّج شاذ) */
...
.brand-logo{width:38px; height:38px; border-radius:11px; background:var(--ink);
  display:flex; align-items:center; justify-content:center; box-shadow:var(--shadow-sm); flex:none;}
.brand-logo svg{width:21px; height:21px; color:#E8C77A;}
.brand-name{font-family:'Cairo',sans-serif; font-weight:700; font-size:1.35rem; color:var(--ink);
...
/* البطل: بطاقة الأونصة بعرض كامل */
.hero-grid{display:grid; grid-template-columns:1fr; gap:16px;}
.hero-card{position:relative; border-radius:22px; padding:1.8rem 1.9rem; overflow:hidden;
  background:var(--surface); border:1px solid var(--line); box-shadow:var(--shadow); animation:fadeUp .7s ease both;}
.hero-card::after{content:""; position:absolute; inset-inline-end:-40px; top:-50px; width:220px; height:220px;
  background:radial-gradient(circle, rgba(156,122,30,.10), transparent 70%); pointer-events:none;}
.hero-kicker{font-size:.8rem; letter-spacing:.5px; color:var(--muted); font-weight:700; margin-bottom:.8rem;}
...
.hero-cur{display:inline-block; font-family:'Plus Jakarta Sans',sans-serif; font-weight:800; font-size:.95rem;
  color:var(--gold-ink); background:var(--gold-soft); border-radius:8px; padding:.15rem .6rem; margin-inline-start:.6rem; vertical-align:middle;}
.hero-usd{margin-top:.8rem; font-family:'Plus Jakarta Sans',sans-serif; font-weight:700; font-size:1.3rem;
  color:var(--ink2); font-variant-numeric:tabular-nums;}
.hero-usd .u{font-family:'Tajawal',sans-serif; font-size:.82rem; color:var(--muted); font-weight:600; margin-inline-start:.4rem;}
.hero-meta{margin-top:1.1rem; padding-top:.9rem; border-top:1px dashed var(--line); font-size:.82rem; color:var(--muted);}
...
/* الشارة في التدفق (فوق الأيقونة، بدون تراكب) */
.crown{display:inline-block; font-size:.6rem; letter-spacing:.5px; font-weight:800;
  color:#1A1306; background:#E8C77A; padding:3px 9px; border-radius:20px; margin-bottom:.7rem;}
.ic{width:40px; height:40px; border-radius:12px; display:flex; align-items:center; justify-content:center;
  background:var(--line2); color:var(--gold); margin-bottom:.85rem;}
...
/* البطاقة السوداء المميزة = عيار 24 */
