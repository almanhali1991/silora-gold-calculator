# app.py — سيلورا جولد | لوحة أسعار نقية (فاتحة، RTL، متجاوبة، تحديث كل 60ث)
import streamlit as st
from urllib.parse import quote
import theme
import gold_service as g

# ===== أيقونة تبويب المتصفح (favicon) مطابقة للشعار =====
_FAV = ('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">'
        '<rect width="32" height="32" rx="8" fill="#18181B"/>'
        '<path d="M10 12h12l2.5 4.5L16 26 7.5 16.5Z" fill="none" stroke="#E8C77A" stroke-width="2" stroke-linejoin="round"/>'
        '<path d="M7.5 16.5h17M13 12l-3 4.5 6 9.5 6-9.5-3-4.5" fill="none" stroke="#E8C77A" stroke-width="1.4" stroke-linejoin="round"/>'
        '</svg>')
...
# ===== تحديث تلقائي كل 60 ثانية (إن توفرت المكتبة؛ وإلا يعمل الموقع عاديًا) =====
try:
...
# ===== أدوات تنسيق آمنة (لا رمز عملة ملتصق برقم داخل نص RTL) =====
...
LOGO = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M6 4h12l3 5-9 11L3 9z"/><path d="M3 9h18M9 4 6 9l6 11 6-11-3-5"/></svg>'
...
# ===== البطل: الأونصة (الكبير = دولار، الصغير = ريال) =====
...
if price_24:
    ...
    <div class="hero-grid">
      <div class="hero-card">
        ...
        <div class="hero-usd">{f0(OUNCE_SAR)}<span class="u">ريال سعودي / أونصة</span></div>
...
# ===== صف بطاقات الأعيرة (الريال فوق الدولار — ترتيب ثابت) =====
...
if price_24:
    cards = "".join([
        kcard(IC["star"], "عيار 24 · سبائك",        GRAM["عيار 24"], GRAM_USD["عيار 24"], featured=True, badge="الأكثر تداولاً", delay=".04s"),
        kcard(IC["bar"],  "عيار 22 · خليجي",        GRAM["عيار 22"], GRAM_USD["عيار 22"], delay=".10s"),
        kcard(IC["gem"],  "عيار 21 · مشغولات",      GRAM["عيار 21"], GRAM_USD["عيار 21"], delay=".16s"),
        kcard(IC["sar"],  "عيار 18 · زينة",         GRAM["عيار 18"], GRAM_USD["عيار 18"], delay=".22s"),
    ])
...
NOTES = {"عيار 24": ("الأكثر تداولاً", "gold"), "عيار 22": ("خليجي", "gray"),
         "عيار 21": ("مشغولات", "gray"), "عيار 18": ("زينة", "gray")}
